...
with open("/app/tutors_django/apps/games/chess/move_to_int_latest_122", "rb") as file:
    move_to_int = pickle.load(file)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ChessModel(num_classes=len(move_to_int))
model.load_state_dict(torch.load("/app/tutors_django/apps/games/chess/latest_chess_model_122.pth", map_location=device))
model.to(device)
model.eval()
